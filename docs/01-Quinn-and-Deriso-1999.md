...
which is basically the mid-point length plus whatever the growth increment is. 
...
The plots below this show a subset of $\alpha_l$ arising from some gamma distribution.
...
for(i in 1:length(alpha_l)) {
  if(i == 1) plot(density(rgamma(1e4, alpha_l[i], beta)), xlim = c(0, 40), xlab = "Size", main = "Growth Transitions")
  else lines(density(rgamma(1e4, alpha_l[i], beta)))
}
```
